...
@property (nonatomic, strong) NSDate *nowMonthFirstDate;
@property (nonatomic, strong) NSDate *nowMonthLastDate;
...
@implementation KalLogic_ipad

@synthesize baseDate, fromDate, toDate, daysInSelectedMonth, daysInFinalWeekOfPreviousMonth, daysInFirstWeekOfFollowingMonth,nowMonthFirstDate,nowMonthLastDate;
...
- (id)initForDate:(NSDate *)date
{
  if ((self = [super init]))
  {
    if (monthAndYearFormatter != nil)
    {
        monthAndYearFormatter = nil;
    }
...
- (void)retreatToPreviousMonth
{
  ...
  AppDelegate_iPad * appDelegate = (AppDelegate_iPad *)[[UIApplication sharedApplication] delegate];
    ...
    c.month = -1;
    appDelegate.mainView.timersheetView.calendarView.sel_weekDate = [[[NSCalendar currentCalendar] dateByAddingComponents:c toDate:appDelegate.mainView.timersheetView.calendarView.sel_weekDate options:0] cc_dateByMovingToFirstDayOfTheMonth];
...
- (NSArray *)calculateDaysInFinalWeekOfPreviousMonth
{
  ...
  for (int i = n - (numPartialDays - 1); i < n + 1; i++)
    [days addObject:[KalDate_ipad dateForDay:i month:(int)c.month year:(int)c.year]];
...
- (void)recalculateVisibleDays
{
  ...
  KalDate_ipad *from = [self.daysInFinalWeekOfPreviousMonth count] > 0 ? [self.daysInFinalWeekOfPreviousMonth objectAtIndex:0] : [self.daysInSelectedMonth objectAtIndex:0];
  KalDate_ipad *to = [self.daysInFirstWeekOfFollowingMonth count] > 0 ? [self.daysInFirstWeekOfFollowingMonth lastObject] : [self.daysInSelectedMonth lastObject];
  self.fromDate = [[from NSDate] cc_dateByMovingToBeginningOfDay];
  self.toDate = [[to NSDate] cc_dateByMovingToEndOfDay];
    
  KalDate_ipad *fromMonth = [self.daysInSelectedMonth objectAtIndex:0];
  KalDate_ipad *toMonth = [self.daysInSelectedMonth lastObject]; 
  self.nowMonthFirstDate = [[fromMonth NSDate] cc_dateByMovingToBeginningOfDay];
  self.nowMonthLastDate = [[toMonth NSDate] cc_dateByMovingToEndOfDay];  
}
