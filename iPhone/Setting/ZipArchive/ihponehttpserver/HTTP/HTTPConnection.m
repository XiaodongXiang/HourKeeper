	...
	else
	{
		...
		else
		{
			...
			if([ranges count] == 1)
			{
				...
				unsigned int bytesToRead = (int)range.length < READ_CHUNKSIZE ? (int)range.length : READ_CHUNKSIZE;
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	if(tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			if(expectsUpload)
			{
				...
				uint bytesToRead = (int)requestContentLength < POST_CHUNKSIZE ? (int)requestContentLength : POST_CHUNKSIZE;
	...
	else
	{
		...
		if(requestContentLengthReceived < requestContentLength)
		{
			...
			uint bytesToRead = (int)bytesLeft < POST_CHUNKSIZE ? (int)bytesLeft : POST_CHUNKSIZE;
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGE_RESPONSE_BODY)
	{
		...
		if(bytesLeft > 0)
		{
			unsigned int bytesToRead = (int)bytesLeft < READ_CHUNKSIZE ? (int)bytesLeft : READ_CHUNKSIZE;
