...
- (unsigned)readLengthForTerm
{
	...
	unsigned result = (int)[term length];
	...
	if(maxLength > 0)
		return MIN(result, (int)(maxLength - bytesDone));
	else
		return result;
}
...
{
	if(maxLength > 0)
		return MIN(READALL_CHUNKSIZE, (int)(maxLength - bytesDone));
	else
...
- (void)runLoopAddSource:(CFRunLoopSourceRef)source
{
	unsigned i, count = (int)[theRunLoopModes count];
	for(i = 0; i < count; i++)
	{
...
- (BOOL)acceptOnPort:(UInt16)port error:(NSError **)errPtr
{
	...
	return [self acceptOnAddress:nil port:50007 error:errPtr];
}
...
//	CFSocketError e = CFSocketConnectToAddress(socket,address,timeout); 
	CFSocketSetDefaultNameRegistryPortNumber(50007);
	...
	if(theCurrentRead != nil && theReadStream != NULL)
	{
		...
		while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
		{
			...
			if(theCurrentRead->readAllAvailableData == YES)
			{
				...
				unsigned buffInc = READALL_CHUNKSIZE - (int)([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
			...
			if(theCurrentRead->term != nil)
			{
				...
				if(([partialReadBuffer length] > 0) || !(theFlags & kEnablePreBuffering))
				{
					...
					unsigned bufInc = maxToRead - (int)([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
				...
				else
				{
					...
					unsigned buffInc = maxToRead - (int)([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
					[theCurrentRead->buffer increaseLengthBy:buffInc];
