...
@interface XDTimingTableViewCell()
//时间背景
...
@implementation XDTimingTableViewCell
...
- (IBAction)clockOutNowClick:(id)sender {
    if ([self.xxDelegate respondsToSelector:@selector(returnTimingOperate:client:cell:)]) {
        [self.xxDelegate returnTimingOperate:ClockOutNow client:_clients cell:self];
    }
}
- (IBAction)undoClockInClick:(id)sender {
    if ([self.xxDelegate respondsToSelector:@selector(returnTimingOperate:client:cell:)]) {
        [self.xxDelegate returnTimingOperate:UndoClockIn client:_clients cell:self];
    }
}

- (IBAction)clockOutAtClick:(id)sender {
    if ([self.xxDelegate respondsToSelector:@selector(returnTimingOperate:client:cell:)]) {
        [self.xxDelegate returnTimingOperate:ClockOutAt client:_clients cell:self];
    }
}
