...
@interface EditLogViewController_new()
{
...
@implementation EditLogViewController_new
...
//    [backButton setTitle:@"Back" forState:UIControlStateNormal];
    ...
    if ( self.selectLog.notes == nil || [self.selectLog.notes isEqualToString:@""])
    {
...
-(void)setSubViewFrame
{
    ...
    {
        float left = 20;
        _ratePerHourLabel1.left = left;
        _startLabel1.left = left;
        _endLabel1.left = left;
        _duationLabel1.left = left;
        _breaktimeLabel1.left = left;
        _overtimeLabel1.left = left;
        _noteTextV.left = 17;
        _noteLabel.left = _noteLabel.left + 5;
        
        _rateField.left = _rateField.left - 5;
        _startDateLbel.left = _startDateLbel.left - 5;
        _endDateLbel.left = _endDateLbel.left - 5;
        _timeWorkLbel.left = _timeWorkLbel.left - 5;
        _breakTimeLbel.left = _breakTimeLbel.left - 5;
        _taxSwitch.left = _taxSwitch.left - 5;
        
    }
    
    _noteTextV.width = SCREEN_WITH - _noteTextV.left *2;
...
-(void)saveBack
{
    ...
    if (_taxSwitch.on != self.selectLog.overtimeFree.boolValue)
    {
    ...
    //syncing
    //获取该log对用的invoice
    NSMutableArray *dataMarray = [[NSMutableArray alloc] initWithObjects:self.selectLog, nil];
    if ([self.selectLog.isInvoice boolValue] == YES && [[self.selectLog.invoice allObjects] count] > 0)
    {
        Invoice *sel_invoice = [[self.selectLog.invoice allObjects] objectAtIndex:0];
    ...
    self.selectLog.sync_status = [NSNumber numberWithInt:0];
    [context save:nil];
    
    //parse update local
    AppDelegate_iPhone *appDelegate_iPhone = (AppDelegate_iPhone *)[[UIApplication sharedApplication]delegate];
    [appDelegate_iPhone.parseSync updateLogFromLocal:self.selectLog];
